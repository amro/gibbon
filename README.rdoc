...
== Stuff You Probably Shouldn't Use

You should really use API keys but if you must use login (deprecated in MailChimp API):

    gb = Gibbon::API.new
    response = gb.login({:username => username, :password => password})
    gb.apikey = response.gsub('"', '')
